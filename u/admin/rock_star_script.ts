  ...
  async function uploadDirRecursive(currentDir: string, currentS3Path: string) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // });
        console.log(encodeURIComponent(s3Key))
        const response = await fetch(`localhost:8000/api/w/${workspace}/job_helpers/upload_git_repo_file_to_instance_storage?file_key=${encodeURIComponent(s3Key)}`, {
          method: 'POST',
          headers: {
            'Authorization': 'Bearer nnVZDzf64Tl2fVUCJLx3Ac8XpfGwcQvN',
            'Content-Type': 'application/octet-stream'
          },
          body: fileContent
        });
        if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText} (${await response.text()})`);
